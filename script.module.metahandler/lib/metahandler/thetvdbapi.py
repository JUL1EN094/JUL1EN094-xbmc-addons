...
class TheTVDB(object):
    ...
    def get_language(self) :
        tvdb_language = __addon__.getSetting('tvdb_language')
    ...
    def get_matching_shows(self, show_name):
        ...
        url = "%s/GetSeries.php?%s&language=%s" % (self.base_url, get_args, self.lang)
        print url
    ...
    def get_show(self, show_id):
        ...
        url = "%s/series/%s/all/%s.xml" % (self.base_key_url, show_id,self.lang)
    ...
    def get_show_by_imdb(self, imdb_id):
        """Get the show object matching this show_id."""
        #url = "%s/series/%s/%s.xml" % (self.base_key_url, show_id, "el")
        url = "%s/GetSeriesByRemoteID.php?imdbid=%s&language=%s" % (self.base_url, imdb_id, self.lang)
        data = StringIO(urllib.urlopen(url).read())
        temp_data = data.getvalue()
        tvdb_id = ''
        if temp_data.startswith('<?xml') == False:
    ...
    def get_episode(self, episode_id):
        ...
        url = "%s/episodes/%s/%s.xml" % (self.base_key_url, episode_id, self.lang)
        data = urllib.urlopen(url)
        print url
        episode = None
        try:
    ...
    def get_episode_by_airdate(self, show_id, aired):
        ...
        url = "%s/GetEpisodeByAirDate.php?apikey=1D62F2F90030C444&seriesid=%s&airdate=%s&language=" % (self.base_url, show_id, aired, self.lang)
        data = StringIO(urllib.urlopen(url).read())
